...
export default function DoctorDashboard() {
    ...
    if (sensing?.fall_detected) {
      setActiveAlerts((prev) => {
        ...
        const alreadyExists = prev.some(
          (a) =>
            a.severity === sensing.severity &&
            a.spike_count === sensing.spike_count,
        );
        if (alreadyExists) return prev;
        return [
          { ...sensing, time: new Date().toLocaleTimeString() },
          ...prev,
        ].slice(0, 20);
      });
      setDismissed(false);
  ...
  const selectedPatient =
    vitals.find((v) => v.patient_id === selectedId) || vitals[0];
      ...
      <div className="bg-white rounded-lg shadow-sm p-6 mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Live patient monitoring via ISAC sensing
          </p>
        </div>
          ...
          {activeAlerts.length > 0 && (
            <div className="flex items-center gap-2 bg-red-50 border border-red-200 px-3 py-1 rounded-full">
              ...
              <span className="text-red-600 text-sm font-medium">
                {activeAlerts.length} fall events
              </span>
            </div>
          )}
          {connected ? (
              ...
              {vitals.map((v) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-sm font-medium border transition
                    ${
                      selectedId === v.patient_id ||
                      (!selectedId && vitals[0]?.patient_id === v.patient_id)
            ...
            <>
              ...
              <div className="bg-white rounded-lg shadow-sm p-4 mb-6 flex gap-6 text-sm text-gray-600">
                ...
                <span>
                  Last update:{" "}
                  <strong className="text-gray-900">
                    {new Date(selectedPatient.timestamp).toLocaleTimeString()}
                  </strong>
                </span>
              </div>
            </>
            ...
            {alerts.length === 0 ? (
              ...
              <div className="space-y-3 max-h-screen overflow-y-auto">
                {alerts.map((a, i) => (
                  <div
                    key={i}
                    className={`border-l-4 p-3 rounded ${severityColor(a.severity)}`}
                  >
                    <div className="flex items-center justify-between">
                      <p className="font-medium text-sm">{a.patient_id}</p>
                      <span
                        className={`text-xs font-bold px-2 py-0.5 rounded-full ${severityBadge(a.severity)}`}
                      >
                        {a.severity}
                      </span>
                    </div>
